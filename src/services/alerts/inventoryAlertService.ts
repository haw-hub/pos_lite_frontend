...
export const inventoryAlertService = {
  ...
  initialize: async (): Promise<void> => {
    await localNotificationService.configureHandler();
    await localNotificationService.configureInventoryChannel();
    const granted = await localNotificationService.ensurePermission(true);
    if (!granted) return;
